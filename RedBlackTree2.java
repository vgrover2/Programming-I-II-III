...
public class RedBlackTree<T extends Comparable<T>> implements SortedCollectionInterface<T> {
  ...
  /**
   * Takes a reference to a newly added red node as its only parameter. Method may be called
   * recursively, in which case the input parameter may reference a different red node in the tree
   * that potentially has a red parent node. This method resolves red child under red parent red
   * black tree property violations that are introduced by inserting new nodes into a red black
   * tree. While doing so, all other red black tree properties must also be preserved.
   * 
   * @param node new Red node inserted into the tree
   */
  private void enforceRBTreePropertiesAfterInsert(Node<T> node) {
    if (node.parent == null) { // node added is the root, so coloring it black
      node.isBlack = true;
      return;
    }
    ...
    if (node.parent.isBlack == true) {
      ...
      if (gParent != null && parent.isLeftChild()) {
        uncle = gParent.rightChild;
      } 
      else if (gParent != null && !parent.isLeftChild()) {
      ...
      if (node.isLeftChild() && parent.isLeftChild()) {
        ...
        rotate(node, parent);
        rotate(gParent.leftChild, gParent);
        gParent.isBlack = false;
        node.isBlack = true;
      } 
      else if (!node.isLeftChild() && !parent.isLeftChild() && parent.parent != null) {
        rotate(parent, gParent);
        gParent.isBlack = false;
        node.isBlack = false;
      }
        ...
        rotate(node, parent);
        rotate(gParent.rightChild, gParent);
        gParent.isBlack = false;
        node.isBlack = true;
      }
      return;
   ...
   * 
   * @param data the data value to test for*@return true if*data* is in the tree,false if it is not
   *             in the tree
   */
